...
public class ImportReconciliationMatchedInternalEntryFunctions {
	...
	public static Long insertInternalEntry(Connection sourceConn, Connection targetConn, Map<String, Object> row) throws Exception {
		...
		if(row.get("matchedEntriesIds") == null) {
			
			return null;
		}
		
		String schema = getEntrySchema(sourceConn, (Long) row.get("matchedEntriesIds"));
		...
		Map<String, Object> internalEntryData = readInternalEntry(sourceConn, (Long) row.get("matchedEntriesIds"), schema);
		...
		if(schema.toString().equals("ACCOUNT")) {
			...
			statement.setLong(3, (Long) row.get("matchedEntriesIds"));
			...
			statement.setLong(6, (Long) row.get("matchedEntriesIds"));
			...
			statement.setLong(4, (Long) row.get("matchedEntriesIds"));
			...
			statement.setLong(7, (Long) row.get("matchedEntriesIds"));
		}
