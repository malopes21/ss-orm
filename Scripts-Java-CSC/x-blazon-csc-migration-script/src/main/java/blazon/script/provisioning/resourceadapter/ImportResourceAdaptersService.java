...
public class ImportResourceAdaptersService {
	
	
	private final static Logger LOGGER = Logger.getLogger(ImportResourceAdaptersService.class.getName());
	...
	public static void execute() {
		...
		LOGGER.log(Level.INFO, "Iniciando importação de ResourceAdapters ...");
		...
		try {
			...
			List<Map<String, Object>> rows = ImportResourceAdaptersFunctions.readSourceResourceAdapters(limit);
			...
			while(!rows.isEmpty()) {
				...
				ImportResourceAdaptersFunctions.saveTargetResourceAdapters(rows);
				
				rows = ImportResourceAdaptersFunctions.readSourceResourceAdapters(limit);
		...
		LOGGER.info("Finalizado importação de ResourceAdapters!");
	}
	...
	private static void createImportValidationField() throws Exception {
		...
		try {
			...
			String sql = "alter table ResourceAdapter add column _imported_ int default 0";
	...
	private static void deleteImportValidationField() throws Exception {
		...
		String sql = "alter table ResourceAdapter drop column _imported_";
