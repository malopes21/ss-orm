...
public class ImportProvisioningTaskDetailsService {
	...
	public static void execute() {
		...
		String propertyFileName = "properties/provisioning/importProvisioningTaskDetails.properties";
		...
		try {
			...
			List<Map<String, Object>> rows = ImportProvisioningTaskDetailsFunctions.read(limit, offset);
			
			int total = rows.size();
			
			while(!rows.isEmpty()) {
				
				ImportProvisioningTaskDetailsFunctions.save(rows);
				...
				rows = ImportProvisioningTaskDetailsFunctions.read(limit, offset);
