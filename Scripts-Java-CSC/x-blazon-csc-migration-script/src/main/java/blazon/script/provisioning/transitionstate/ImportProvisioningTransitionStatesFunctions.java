...
class ImportProvisioningTransitionStatesFunctions {
	...
	static List<Map<String, Object>> readSourceProvisioningTransitionStates(int limit) throws Exception {
	...
	public static void saveTargetProvisioningTransitionStates(List<Map<String, Object>> rows) throws Exception {
		...
		for (Map<String, Object> row : rows) {
			...
			try {
				...
				LOGGER.log(Level.SEVERE, String.format("Erro ao importar provisioning entry transition state com id %s", row.get("id")));
				throw e;
			}
		}
	...
	private static void saveProvisioningTransitionState(Connection conn, Map<String, Object> row) throws SQLException {
		...
		if(row.get("provisioningEntry_id") != null) { statement.setLong(8, (Long)row.get("provisioningEntry_id")); } else { statement.setObject(8, null); }
	...
	private static void setImportedProvisioningTransitionState(Connection conn, Map<String, Object> row) throws Exception {
		...
		String sql = "update ProvisioningEntryTransitionState set _imported_ = 1 where id = ?";
		statement = conn.prepareStatement(sql);
