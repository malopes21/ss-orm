...
class ImportMembershipEntitlementsFunctions {
	...
	public static void save(List<Map<String, Object>> rows) throws Exception {
		...
		for(Map<String, Object> row: rows) {
			...
			try {
				...
				LOGGER.log(Level.ERROR, String.format("Erro ao importar membership entitlement com id %s", row.get("id")));
				throw e;
			}
		}
	...
	private static void saveMembershipEntitlement(Connection conn, Map<String, Object> row) throws SQLException {
		...
		LOGGER.log(Level.INFO, "Enviando comando SQL para importar membership entitlement com id " + row.get("id"));
	}
