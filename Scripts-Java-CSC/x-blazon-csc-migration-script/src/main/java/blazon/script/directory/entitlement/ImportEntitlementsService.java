...
public class ImportEntitlementsService {
	...
	public static void execute() {
		...
		try {
			...
			List<Map<String, Object>> rows = ImportEntitlementsFunctions.readSourceEntitlements(limit);
			...
			while(!rows.isEmpty()) {
				...
				ImportEntitlementsFunctions.saveTargetEntitlements(rows);
				
				rows = ImportEntitlementsFunctions.readSourceEntitlements(limit);
	...
	private static void createImportValidationField() throws Exception {
		...
		try {
			...
			String sql = "alter table Entitlement add column _imported_ int default 0";
	...
	private static void deleteImportValidationField() throws Exception {
		...
		String sql = "alter table Entitlement drop column _imported_";
