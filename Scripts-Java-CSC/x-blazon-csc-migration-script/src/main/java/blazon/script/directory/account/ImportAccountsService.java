...
public class ImportAccountsService {
	...
	public static void execute() {
		...
		try {
			...
			List<Map<String, Object>> rows = ImportAccountsFunctions.readSourceAccounts(limit);
			...
			while(!rows.isEmpty()) {
				...
				ImportAccountsFunctions.saveTargetAccounts(rows);
				
				rows = ImportAccountsFunctions.readSourceAccounts(limit);
		...
		LOGGER.info("Finalizado importação de Accounts!");
	}
	...
	private static void createImportValidationField() throws Exception {
		...
		try {
			...
			String sql = "alter table Account add column _imported_ int default 0";
	...
	private static void deleteImportValidationField() throws Exception {
		...
		String sql = "alter table Account drop column _imported_";
