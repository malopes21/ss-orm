...
public class ImportMembershipEntitlementsService {
	...
	public static void execute() {
		...
		try {
			...
			List<Map<String, Object>> rows = ImportMembershipEntitlementsFunctions.read(limit, offset);
			
			int total = rows.size();
			
			while(!rows.isEmpty()) {
				
				ImportMembershipEntitlementsFunctions.save(rows);
				...
				rows = ImportMembershipEntitlementsFunctions.read(limit, offset);
		...
		LOGGER.info("Finalizado importação de Membership entitlements!");
	}
}
