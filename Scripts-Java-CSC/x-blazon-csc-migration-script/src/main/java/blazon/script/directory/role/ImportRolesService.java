...
public class ImportRolesService {
	...
	public static void execute() {
		...
		String propertyFileName = "properties/directory/importRoles.properties";
		...
		try {
			...
			List<Map<String, Object>> rows = ImportRolesFunctions.read(limit, offset);
			
			int total = rows.size();
			
			while(!rows.isEmpty()) {
				
				ImportRolesFunctions.save(rows);
				...
				rows = ImportRolesFunctions.read(limit, offset);
