...
public class ImportBlazonRequestTransitionStatesService {
	...
	public static void execute() {
		...
		String propertyFileName = "properties/request/importRequestStates.properties";
		
		LOGGER.log(Level.INFO, "Iniciando importação de BlazonRequests Transition States ...");
		...
		try {
			...
			List<Map<String, Object>> rows = ImportBlazonRequestTransitionStatesFunctions.read(limit, offset);
			
			int total = rows.size();
			
			while(!rows.isEmpty()) {
				
				ImportBlazonRequestTransitionStatesFunctions.save(rows);
				...
				rows = ImportBlazonRequestTransitionStatesFunctions.read(limit, offset);
		...
		LOGGER.info("Finalizado importação de BlazonRequests Transition States !");
	}
