...
import blazon.script.indexation.directory.entitlement.builder.ImportEntitlementToIndexationBuilder;
import blazon.script.util.ConnectionFactory;

class ImportEntitlementDirectoryIndexationsFunctions {

	private final static Logger LOGGER = Logger.getLogger(ImportEntitlementDirectoryIndexationsFunctions.class.getName());
	...
	public static void save(List<Map<String, Object>> rows) throws Exception {
		...
		for (Map<String, Object> row : rows) {
			...
			try {
				...
				Map<String, Map<String, Object>> schema = ImportAttributeSchemaService.getAttributeSchema(sourceConn, "ENTITLEMENT", (Long) row.get("resource_id"));
				...
				JSONObject entryJson = ImportEntitlementToIndexationBuilder.createEntry(schema, row);
				...
				LOGGER.log(Level.ERROR, String.format("Erro ao importar entitlement com id %s", row.get("id")));
				throw e;
			}
		}
	...
	public static void saveTargetEntitlement(JSONObject entryJson) throws Exception {
		...
		LOGGER.log(Level.INFO, String.format("Comando HTTP POST emitido para importar entitlement com id %s", entryJson.getLong("identifier")));
	}
