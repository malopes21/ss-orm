...
public class ImportResourceDirectoryIndexationsService {
	...
	public static void execute() {
		...
		LOGGER.log(Level.INFO, "Iniciando importação de Resource to Directory indexation ...");
		...
		try {
			...
			List<Map<String, Object>> rows = ImportResourceDirectoryIndexationsFunctions.readSourceResources(limit);
			...
			while(!rows.isEmpty()) {
				...
				ImportResourceDirectoryIndexationsFunctions.saveTargetResources(rows);
				
				rows = ImportResourceDirectoryIndexationsFunctions.readSourceResources(limit);
		...
		LOGGER.info("Finalizado importação de Resource to Directory indexation!");
	}
	...
	private static void createImportValidationField() throws Exception {
		...
		try {
			...
			String sql = "alter table Resource add column _imported2_ int default 0";
	...
	private static void deleteImportValidationField() throws Exception {
		...
		String sql = "alter table Resource drop column _imported2_";
