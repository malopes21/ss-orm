...
public class ImportRoleDirectoryIndexationsService {
	...
	public static void execute() {
		...
		String propertyFileName = "properties/indexation/importRoles.properties";
		
		LOGGER.log(Level.INFO, "Iniciando importação de Indexation roles ...");
		...
		try {
			...
			List<Map<String, Object>> rows = ImportRoleDirectoryIndexationsFunctions.read(limit, offset);
			
			int total = rows.size();
			
			while(!rows.isEmpty()) {
				
				ImportRoleDirectoryIndexationsFunctions.save(rows);
				...
				rows = ImportRoleDirectoryIndexationsFunctions.read(limit, offset);
		...
		LOGGER.info("Finalizado importação de Indexation roles!");
	}
