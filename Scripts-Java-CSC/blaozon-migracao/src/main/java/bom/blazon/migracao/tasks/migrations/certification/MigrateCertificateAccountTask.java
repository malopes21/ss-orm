...
public class MigrateCertificateAccountTask {
	...
	public static void create(Map<String, Object> data, Connection sourceConn, Connection destinationConn) throws ClassNotFoundException, SQLException {
		...
		Long resourceId = Long.valueOf(data.get("resourceId").toString());
		
		Map<String, Object> resource = SearchResource.read(resourceId, sourceConn);
		
		if(resource == null) {
			
			System.out.println(String.format("Detalhes sobre resource não foi encontrado. TaskId: %s", data.get("id")));
			...
			addTaskData(taskId, "type", "ACCOUNT", destinationConn);
